...
class SlackAIAgent {
  ...
  setupExpress() {
    ...
    // Test endpoint exposed for testing and local verification
...
Consider job title, company size, technical background, and budget authority.`,
  ...
  async start() {
    try {
      ...
      const host = "0.0.0.0"; // Explicit host binding required for Render container routing

      this.server = this.app.listen(port, host, () => {
        log.info(`🚀 Express server running on http://${host}:${port}`);
      });
